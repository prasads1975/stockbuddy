...
| Article ID | Field | **Optional, deployment-configurable** identifier for a specific physical unit (item-level SKU); e.g. ART0001. Not every industry/client pre-prints a unique per-unit identifier — see Section 1.7. Where Article ID is not used, **RFID Tag ID** serves as the guaranteed unique per-unit identifier. |
...
Regardless of which mode is active, **RFID Tag ID remains the guaranteed unique identifier for a physical unit** (per the Section 1.6 assumption that every RFID tag is globally unique), and **Barcode remains the key used for product-level grouping and Product Master matching** (FR-21, FR-22, FR-45). Article ID, where used, is supplementary human-readable identification — the app's core workflows (linking, scanning, grouping, matching) do not depend on it being present or unique. This resolves the risk previously tracked as OQ-30.
...
| NFR-56 | Article ID configurability | Article ID usage shall be configurable per deployment as **Not Used / Optional / Mandatory** (see above), independent of the domain-specific field engine (NFR-51–55). This setting shall be stored alongside the field configuration metadata (NFR-54) and persist across app updates and backup/restore (Section 5.9). Changing the setting at runtime shall not retroactively delete any Article ID values already captured. |
...
| FR-80a | P3 | **Domain field reconfiguration**: after first-run setup, the Admin may add, edit, or remove domain-specific fields, and change the Article ID usage toggle, at any time via a new Settings → Field Configuration screen, using the same interface as Step 3 (FR-77i–FR-77o). Changes apply immediately per NFR-52–NFR-53 (existing records get `null` for new fields); changing the Article ID toggle does not retroactively delete previously captured Article ID values (NFR-56). |
...
- Exact QR payload schema for S07 (OQ-01)
...
|---|----------|----------------------------------|
| OQ-01 | P1 | **QR payload schema (critical):** The QR code on each item's label is the sole data-entry mechanism for Individual Linking. The exact payload format must be confirmed before development: field names or positions, delimiter or encoding (JSON, delimited string, GS1, custom), and whether the fixed fields (Product Name, Article ID, Barcode, Category) plus configured domain-specific fields (e.g. Price) and RFID Tag ID are always present. Who owns the QR generation spec — warehouse team or supplier? |
