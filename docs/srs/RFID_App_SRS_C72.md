...
| FR-35 | P1 | The screen shall provide a **Category filter dropdown** ("All" or a specific category), selected *before* scanning starts. This selection defines the session's intentional **scope**: the operator is deliberately counting only that category (or everything, if "All"). The C72 antenna still physically scans and collects all RFID tags in range regardless of the selected category — the raw scan data is never restricted at the hardware or database level — but **Available/Missing are computed only against the selected category's master items** (see FR-46). *(v3.18 — supersedes the earlier "display filter only, no scope effect" framing from v2.6/v3.17.)* |
...
| FR-46 | P2 | The category filter selected on the scanning screen shall carry over to the results screen as the default view. **The filter control is interactive only when the session was scanned with "All" selected** — in that case the user may change the category filter at any time without re-scanning, and Available/Missing reflect the real computed counts for whichever category is chosen. **If the session was scanned with a specific category, the filter control is locked** (shown as a read-only label naming that category, not an interactive dropdown) — offering other categories would only produce a meaningless 0/0 result (that category was never the operator's intent for this session), so the control is not presented as a choice at all. **Excess is never affected by the category filter** — it reflects tags with no master record at all (genuinely unregistered), not tags outside today's scope — and is always shown at its true count. The underlying raw scan data (all tags collected by the antenna) is still preserved in full in the database for audit/export purposes, even when the Available/Missing display is scope-locked. *(v3.19 — refines v3.18: rather than an interactive control that reads 0/0 for most choices, the control itself is locked/hidden when the session was scoped to one category.)* |
...
| 3.18 | Jul-2026 | **Category filter reclassified from a pure display filter to a session-scoping filter (FR-35, FR-46 rewritten).** Implementation testing surfaced that the category selected on Start Inventory Scan was doing nothing end-to-end (never persisted, never applied) — while fixing that wiring bug, the intended semantics were clarified directly with the client-side stakeholder: the category chosen **before** scanning defines what the operator is intentionally counting for that session, not merely a post-hoc display lens. **FR-35** now states the selection scopes Available/Missing computation to that category (the antenna itself is still unrestricted — raw scan data is never filtered at the hardware/DB level). **FR-46** now states that switching the Results-screen filter to the session's own category (or "All", equivalent for that session) shows real counts, but switching to any *other* category reads **0 Available / 0 Missing** — that category was never the operator's intent — while **Excess is always shown at its true, unscoped count** (it means "no master record at all," not "outside today's category"). This supersedes the v2.6 "post-scan display filter only" framing and the FR-46 wording introduced when the Results-screen filter control was first built earlier in the same work session. Implementation: `InventoryRepository.computeSessionStats()` (live scanning KPIs) and `ResultsViewModel.applyFilter()` (post-stop KPIs) both now apply this scope-lock. No FRs renumbered. |
| 3.19 | Jul-2026 | **Results-screen category filter control locked (not just its output) when the session was scoped to one category (FR-46 refined).** v3.18 kept the filter dropdown fully interactive and relied on the 0/0 result to signal "not applicable" for any category besides the session's own. On reflection this was poor UX — a control that mostly leads to a dead end. FR-46 now specifies the control itself is only interactive when the session was scanned with "All" selected; otherwise it's replaced with a read-only label naming the locked category, and no other choice is offered. Implementation: `ResultsViewModel` exposes a new `sessionScope` LiveData; `ResultsFragment` swaps between the interactive `Spinner` and a plain `TextView` (`tvCategoryLocked`) in `fragment_results.xml` based on it. `ResultsViewModel.applyFilter()`'s 0/0 scope-lock logic is retained as a defensive fallback but is no longer reachable through the UI in the locked case. No FRs renumbered. |
