...
| ~~QR scanning screen (S07)~~ | FR-01–04, FR-11 | **NOW IN SCOPE / implemented (v… )** — QR Code Linking (S07) is built using the **C72 2D imager** (not the camera): scan → Individual Linking pre-filled → Save; back returns to the scanner; a QR-flow save re-arms the scanner (FR-11). **Device-only** — on the emulator the imager is unavailable, so S07 shows the FR-81a "reader unavailable" state and cannot scan. Payload = JSON (OQ-01 resolved). The label-printing/QR-generation tool is the deploying party's responsibility. |
