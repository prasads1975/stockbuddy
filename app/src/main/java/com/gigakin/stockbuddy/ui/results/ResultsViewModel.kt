...
class ResultsViewModel(
    ...
    fun load(sessionId: Long) = viewModelScope.launch {
    ...
    private fun applyFilter() {
        ...
        filteredResults.value = if (filter == null) all else all.filter { it.item.categoryName == filter || it.status == InventoryRepository.Status.EXCESS }
    }
